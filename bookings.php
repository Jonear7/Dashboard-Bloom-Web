...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
 ...
 // Inside the existing code block handling check-out
if (isset($_POST["submit_checkout"])) {
    ...
    if ($stmt_insert === false) {
        ...
        // Bind parameters
        mysqli_stmt_bind_param($stmt_insert, 'isdsis', $booking_id, $booking_details['checkin_date'], $checkout_date, $booking_details['total_price'], $booking_details['room_number'], $status);
    ...
    if (isset($_POST['submit_add'])) {
        ...
        if ($availability['count'] > 0) {
            $error_message = "The selected room is not available for the specified dates.";
        } else {
...
$query_rooms = "SELECT room.room_id, room.room_number, rmtype.type_name FROM room 
                JOIN rmtype ON room.rmtype_id = rmtype.rmtype_id 
    ...
    <div class="flex-1 p-8">
        <div class="container mx-auto py-8">
            ...
            <?php if(isset($_SESSION['success_message'])): ?>
                <div id="success-message" class="bg-green-500 text-white p-4 mb-4"><?php echo $_SESSION['success_message']; unset($_SESSION['success_message']); ?></div>
            <?php endif; ?>
        ...
        <div class="bg-white p-8 rounded shadow-lg w-1/2 text-black">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                ...
                <div class="mb-4">
                    ...
                    <select name="room_id" id="room_id" class="bg-gray-200 text-black font-bold py-2 px-4 rounded w-full" required>
                        <?php foreach ($rooms as $room): ?>
                            <option value="<?php echo $room['room_id']; ?>"><?php echo $room['room_number'] . ' (' . $room['type_name'] . ')'; ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="mb-4">
                    <label for="checkin_date" class="block text-sm font-bold mb-2">Check-in Date:</label>
                    <input type="date" name="checkin_date" id="checkin_date" class="bg-gray-200 text-black font-bold py-2 px-4 rounded w-full" required>
                </div>
                <div class="mb-4">
                    <label for="checkout_date" class="block text-sm font-bold mb-2">Check-out Date:</label>
                    <input type="date" name="checkout_date" id="checkout_date" class="bg-gray-200 text-black font-bold py-2 px-4 rounded w-full" required>
                </div>
